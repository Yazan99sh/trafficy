...
class CaptainsStateManager {
  ...
  CaptainsStateManager(this._homeService, this._captainsService);

  void getUsers(CaptainsScreenState screenState) {
    ...
    _homeService.getUsersCount().then((value) {
      if (value.hasError) {
        ...
        value as UsersModel;
        _stateSubject.add(CaptainsLoadedState(screenState, value.data));
      }
    });
  }

  void createCaptainAccount(
      CaptainsScreenState screenState, CreateCaptainAccount request) {
    _stateSubject.add(LoadingState(screenState));
